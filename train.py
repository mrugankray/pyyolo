...
def train(epochs):
    for epoch in range(epochs):
        ...
        for trn_i, trn_sample in enumerate(train_loader):
            ...
            if train_on_gpu:
                trn_img = trn_img.type(torch.DoubleTensor)
                #output_tnsr = output_tnsr.type(torch.DoubleTensor)
            ...
            if train_on_gpu:
                pc_pred_global_list = pc_pred_global_list.type(torch.DoubleTensor)
                pc_inp_tnsr_global_list = pc_inp_tnsr_global_list.type(torch.DoubleTensor)
                class_scores_global_list = class_scores_global_list.type(torch.DoubleTensor)
                orginal_class_global_list = orginal_class_global_list.type(torch.DoubleTensor)
                coord_pred_global_list = coord_pred_global_list.type(torch.DoubleTensor)
                coord_inp_global_list = coord_inp_global_list.type(torch.DoubleTensor)
            ...
            '''print(pc_pred_global_list)
            print(pc_inp_tnsr_global_list)
            print(class_scores_global_list)
            print(orginal_class_global_list)
            print(coord_pred_global_list)
            print(coord_inp_global_list)'''
            ...
            with torch.no_grad():
                ...
                for val_img, val_sample in enumerate(val_loader):
                    ...
                    if train_on_gpu:
                        val_img = val_img.type(torch.DoubleTensor)
                        #output_tnsr_val = output_tnsr_val.type(torch.DoubleTensor)
                    ...
                    if train_on_gpu:
                        pc_pred_global_list_val = pc_pred_global_list_val.type(torch.DoubleTensor)
                        pc_inp_tnsr_global_list_val = pc_inp_tnsr_global_list_val.type(torch.DoubleTensor)
                        class_scores_global_list_val = class_scores_global_list_val.type(torch.DoubleTensor)
                        orginal_class_global_list_val = orginal_class_global_list_val.type(torch.DoubleTensor)
                        coord_pred_global_list_val = coord_pred_global_list_val.type(torch.DoubleTensor)
                        coord_inp_global_list_val = coord_inp_global_list_val.type(torch.DoubleTensor)
                ...
                for test_i, test_sample in enumerate(test_loader):
                    ...
                    if train_on_gpu:
                        ...
                        #output_tnsr_test = output_tnsr_test.type(torch.DoubleTensor)
                    ...
                    if train_on_gpu:
                        pc_pred_global_list_test = pc_pred_global_list_test.type(torch.DoubleTensor)
                        pc_inp_tnsr_global_list_test = pc_inp_tnsr_global_list_test.type(torch.DoubleTensor)
                        class_scores_global_list_test = class_scores_global_list_test.type(torch.DoubleTensor)
                        orginal_class_global_list_val = orginal_class_global_list_test.type(torch.DoubleTensor)
                        coord_pred_global_list_test = coord_pred_global_list_test.type(torch.DoubleTensor)
                        coord_inp_global_list_test = coord_inp_global_list_test.type(torch.DoubleTensor)
